 ...
 *
 * Algolia is the one boundary the suites cannot keep real: it needs paid
 * credentials, it is a shared mutable index, and it fails for reasons unrelated
 * to the change under test. The SDK ships no double of its own.
 *
 * Standing in at the SDK's `setHttpClient()` seam leaves everything in front of
 * Algolia real — routes, permission checks, validation, option writes and the
 * mapping from an SDK failure to an HTTP status all still execute.
 */
final class Mock_Algolia_Http_Client implements HttpClientInterface {
	 ...
	 *
	 * The client never receives this mode — it tells whoever installs the client
	 * to leave the SDK's real transport alone. Only the smoke suite asks for it.
	 */
	...
	public function sendRequest( RequestInterface $request, $timeout, $connect_timeout ): ResponseInterface { // phpcs:ignore SlevomatCodingStandard.Functions.UnusedParameter.UnusedParameter, Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
		...
		// The plugin validates credentials by asking Algolia for the key's ACL.
